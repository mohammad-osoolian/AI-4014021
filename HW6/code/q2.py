...
player, opponent = 'X', 'O'
results =  [[[0, 0] for _ in range(3)] for i in range(3)]
        ...
        for row in range(3):
            for col in range(3):
                if board[row][col] == '_':
                    board[row][col] = 'O'
                    if checkWin(board):
                        results[row][col] = [results[row][col][0]+1, results[row][col][1]+1]
                    elif checkCanWin(board, player):
                        results[row][col] = [results[row][col][0]-1, results[row][col][1]+1]
                    elif checkCanWin(board, opponent):
                        results[row][col] = [results[row][col][0]+1, results[row][col][1]+1]
                    else:
                        results[row][col] = [results[row][col][0], results[row][col][1]+1]
                    board[row][col] = '_'
    best = -1000000000
    cell = [-1, -1]
    for row in range(3):
        for col in range(3):
            if results[row][col][0]>best:
                best = results[row][col][0]
                cell = [row, col]
